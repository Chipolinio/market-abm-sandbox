...
const noopComplete = vi.fn(async (_before: string, _action: string) => undefined);
...
describe("SimulationControlStrip", () => {
  ...
  it("disables_step_unless_paused", () => {
    render(<SimulationControlStrip state="RUNNING" onActionComplete={noopComplete} />);
    expect((screen.getByRole("button", { name: "Шаг" }) as HTMLButtonElement).disabled).toBe(
      true,
    );
  ...
  it("disables_reset_when_running", () => {
    ...
    const reset = screen.getByRole("button", { name: "Сброс" }) as HTMLButtonElement;
